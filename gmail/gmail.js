        ...
        for await (let message of messages) {
            ...
            if (finalCode) {
                ...
                if (config.isWriteHtml) {
                    const intro = ` <p style="color:red; font-size: 16px;"> <br/> Disney+ 一次性密码 6位数字验证码如下 (请等待2分钟后在刷新页面保证获取到最新的验证码 系统每30秒更新一次): <br/><br/> 输入6位数字验证码成功后有时会提示修改密码(网页显示的提示信息为: Create a new password ) 请务必不要修改密码, 否则一律封号. 请谨慎操作! <br/><br/> 输入6位数字验证码成功后 直接重新打开 <a href="https://www.disneyplus.com/zh-hans/home" target="_blank"> https://www.disneyplus.com/zh-hans/home </a> 即可 \n </p>`
                    ...
                    console.log("fspromises.writeFile content: ", content);
...
async function getDisneyCode() {
    ...
    config.searchGmail.subject = '{您的一次性密码 subject:Your one-time passcode}';
